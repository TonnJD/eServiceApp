...
export class PostDataService {
  ...
  UpdatePendingToInprogress(serno, planid) {
    return new Promise((resovle, reject) => {
      let option: any = new HttpHeaders({ 'Content-Type': 'application/json' });

      this.http.get(this.apiServer_url + '/API/WebService.asmx/UpdatePendingToInprogress?serno=' + serno + '&planid=' + planid, option).subscribe(data => {
        resovle(data);
      }, error => {
        reject(error)
      });
